...
def generate_User_stats(ulist,logging=0):
...
def check_uid_for_tag(ulist,tag,logging=0):
...
def get_amount_for_tag(ulist,tag,logging=0):
    '''Returns a list of IDs of users in ulist that have a given tag and how many have the given tag in the format ([users],amount)'''
    users=[]
    s=0
    for user in ulist:
...
def generate_users_file(filename="testUsers.txt",logging=0):
...
def read_users_from_file(filename="testUsers.txt",logging=0):
...
def generate_mock_users(amount=1000):
...
if __name__=="__main__":
    ...
    #for i in range(5):
        #print(users_list[i])
    lista=generate_User_stats(users_list,0)
    print()
    #check_uid_for_tag(users_list,3,1)
    #generate_users_file()
    raw_users=read_users_from_file()
    print()